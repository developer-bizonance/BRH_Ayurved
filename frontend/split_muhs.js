...
const generatePage = (year, filename) => {
  const pagePath = `src/app/biometric-attendance/muhs-holiday-${year}/page.tsx`;
  fs.mkdirSync(path.dirname(pagePath), { recursive: true });

  const content = `import PageBanner from "@/components/ui/PageBanner";
import Link from "next/link";

export const metadata = { title: "MUHS Holiday ${year}" };

const files = [
  { name: "MUHS Holiday ${year}", url: "/uploads/biometric_attendance/${filename}" }
];

export default function MUHSHoliday${year}Page() {
  return (
    <div>
      <PageBanner title="MUHS Holiday ${year}" breadcrumbs={[{ label: "Biometric Attendance" }, { label: "MUHS Holiday ${year}" }]} />
...
generatePage(2025, 'MUHS Holiday 202520250626_175530.pdf');
generatePage(2026, 'MUHS Holiday 202620251216_062422.pdf');

// Remove the old combined page
fs.rmSync('src/app/biometric-attendance/muhs-holidays', { recursive: true, force: true });
console.log('Done!');
