...
interface AccordionSectionData {
  category: string;
  files: FileItem[];
  subsections?: AccordionSectionData[];
}
...
function AccordionSection({ section, defaultOpen = false }: { section: AccordionSectionData, defaultOpen?: boolean }) {
  ...
  return (
    <div className="w-full border-b border-gray-200 overflow-hidden">
      ...
      >
        <h3 className="text-xl font-medium text-[#15803d] font-[var(--font-playfair)]">{section.category}</h3>
        <div className={`w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-full transition-all duration-300 ${isOpen ? 'bg-[#e8821a] text-white rotate-180' : 'bg-[#15803d]/10 text-[#15803d] group-hover:bg-[#15803d] group-hover:text-white'}`}>
      ...
      >
        <div className="pb-4 pt-1">
            ...
            <div className="flex flex-col gap-2">
              {section.files.map((file, fileIdx) => (
                ...
                >
                  ...
                  <h4 className="font-medium text-gray-700 text-[15px] leading-snug group-hover:text-[#15803d] transition-colors duration-300">
                    {file.name}
                  </h4>
                </Link>
              ))}
          ...
          {section.subsections && section.subsections.length > 0 && (
            <div className="pl-6 border-l-2 border-[#15803d]/20 mt-2 ml-4">
              <DocumentAccordion sections={section.subsections} defaultOpenIdx={0} />
            </div>
          )}
        </div>
      </div>
...
export default function DocumentAccordion({ sections, defaultOpenIdx = -1 }: DocumentAccordionProps) {
  return (
