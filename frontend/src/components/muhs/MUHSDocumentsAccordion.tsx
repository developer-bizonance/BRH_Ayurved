...
export interface MUHSDocument {
  name: string;
  url: string;
}

export interface MUHSSection {
  category: string;
  files: MUHSDocument[];
}

function AccordionSection({ section, defaultOpen = false }: { section: MUHSSection, defaultOpen?: boolean }) {
        ...
        <div>
          <div className="flex flex-col gap-2 pb-4 pt-1">
            {section.files.length === 0 ? (
              <p className="text-gray-500 italic px-4 py-2">No documents available.</p>
            ) : (
...
export default function MUHSDocumentsAccordion({ documentsData }: { documentsData: MUHSSection[] }) {
  return (
