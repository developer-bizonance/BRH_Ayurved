...
export const metadata = { title: "Yearly OPD - IPD Record" };
...
export default function YearlyOPDIPDRecordPage() {
  return (
    <div>
      <PageBanner title="Yearly OPD - IPD Record" breadcrumbs={[{ label: "Hospital" }, { label: "Yearly OPD - IPD Record" }]} />
      
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 pt-0">
        <div className="flex flex-col gap-2">
          {yearlyRecordsFiles.map((file, fileIdx) => (
            <Link 
              href={file.url} 
              target="_blank"
              key={fileIdx}
            ...
            >
              ...
              <h4 className="font-medium text-gray-700 text-[16px] leading-snug group-hover:text-[#15803d] transition-colors duration-300">
                {file.name}
              </h4>
            </Link>
          ))}
