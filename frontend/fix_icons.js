...
const newSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 52 72" width="32" height="32">
  <path fill="#f5f5f5" stroke="#ff2a2a" strokeWidth="4" strokeLinejoin="round" d="M8,2 h26 l16,16 v48 a4,4 0 0,1 -4,4 H8 a4,4 0 0,1 -4,-4 V6 a4,4 0 0,1 4,-4 z" />
...
files.forEach(file => {
  ...
  // 1. Replace the SVG block
  const regexSVG = /<div className="flex-shrink-0 mr-4">[\s\S]*?<\/div>/g;
  content = content.replace(regexSVG, `<div className="flex-shrink-0 mr-4">\n${newSvg}\n</div>`);
  
  // 2. Reduce the card padding (p-5 -> p-3)
  content = content.replace(/className="group flex items-center bg-white rounded-md p-5 /g, 'className="group flex items-center bg-white rounded-md p-3 ');
  
  // 3. Reduce the text size (text-[17px] -> text-[15px])
  content = content.replace(/text-\[17px\]/g, 'text-[15px]');

  // 4. For recognition/page.tsx & CollegeDocumentsAccordion.tsx, reduce mb-5 to mb-3 and space-y-10 to space-y-6
  content = content.replace(/className="mb-5"/g, 'className="mb-3"');
  content = content.replace(/className="w-full space-y-10"/g, 'className="w-full space-y-6"');
  
  fs.writeFileSync(filePath, content);
});

console.log("Done fixing icons and layout sizing.");
