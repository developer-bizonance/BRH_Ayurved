...
const data = require('./teaching_staff_biometric_parsed.json');
const pagePath = 'src/app/biometric-attendance/teaching-staff/page.tsx';

const pageContent = `import PageBanner from "@/components/ui/PageBanner";
import Link from "next/link";

export const metadata = { title: "Teaching Staff Attendance" };

const files = ${JSON.stringify(data, null, 2)};
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {files.map((file, idx) => (
            <Link 
              ...
              target="_blank"
              key={idx}
              className="group flex items-center p-6 bg-white shadow-sm border-b-4 border-[#3ca291] hover:shadow-md transition-all duration-300 hover:-translate-y-1 rounded-sm"
            >
              <div className="flex-shrink-0 mr-4">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 52 72" width="32" height="32">
              ...
              <h4 className="font-semibold text-gray-800 text-[15px] leading-snug group-hover:text-[#3ca291] transition-colors duration-300">
                {file.name}
              </h4>
            </Link>
          ))}
...
console.log('Successfully updated teaching staff attendance page.');
